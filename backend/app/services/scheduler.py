...
# Initialize scheduler only if imports succeeded
if AsyncIOScheduler:
...
def start_scheduler():
    ...
    if not scheduler:
        print("Scheduler not available - skipping scheduling.")
        return
    try:
        ...
        if _is_truthy(os.getenv("SELF_PING_ENABLED", "true")):
            ...
            if ping_url:
                ...
                print("WARNING: SELF_PING_ENABLED is true but SELF_PING_URL is empty. Skipping self-ping job.")
